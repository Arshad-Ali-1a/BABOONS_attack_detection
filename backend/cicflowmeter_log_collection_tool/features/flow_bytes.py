...
class FlowBytes:
    ...
    def get_bulk_rate(self, packet_direction):
        if packet_direction == PacketDirection.FORWARD:
            # !! changed forward_bulk_count to forward_bulk_duration in the immediate bottom line.
            if self.feature.forward_bulk_duration != 0:
                return (
                    self.feature.forward_bulk_size / self.feature.forward_bulk_duration
                )
        else:
            # !! changed backward_bulk_count to backward_bulk_duration in the immediate bottom line.
